...
impl<'a, W: Write> Formatter<'a, W> {
        ...
        F: Fn(&V, &V) -> bool,
    {
        ...
        while let Some(mut line_item) = pop_next(self, &mut items) {
                    ...
                    if !ignore_whitespace {
                        ...
                        if let Some(mut last_loc) = last_loc {
                            ...
                            let is_last_doc_comment = matches!(
                                last_comment,
                                Some(CommentWithMetadata {
                                    ty: CommentType::DocBlock,
                                    ..
                                })
                            );
            ...
            if let Some(comment) = line_item.left() {
                if comment.is_line() {
                    last_byte_written = self
                        .find_next_line(last_byte_written)
                        .unwrap_or(last_byte_written);
                }
        ...
        M: FnMut(&mut Self, &'b mut Box<T>) -> Result<Option<(&'b mut Box<T>, &'b mut Identifier)>>,
    {
        ...
        chunks
            .iter_mut()
            .for_each(|chunk| chunk.content.insert(0, '.'));
        ...
        let mut write_attributes = |fmt: &mut Self, multiline: bool| -> Result<()> {
            ...
            if !func.returns.is_empty() {
                ...
                if fmt.inline_config.is_disabled(returns_loc) {
                    ...
                    for function_chunk in returns
                        .iter_mut()
                        .filter(|chunk| chunk.content.starts_with("function("))
                    {
    ...
    fn write_if_stmt(
        ...
        let cond_close_paren_loc = self
            .find_next_in_src(cond.loc().end(), ')')
            .unwrap_or_else(|| cond.loc().end());
...
impl<W: Write> Visitor for Formatter<'_, W> {
    ...
    fn visit_pragma(&mut self, pragma: &PragmaDirective) -> Result<()> {
        match pragma {
            PragmaDirective::Identifier(loc, ident, string) => {
            ...
            PragmaDirective::StringLiteral(loc, ident, string) => {
                return_source_if_disabled!(self, *loc, ';');
            ...
            PragmaDirective::Version(loc, ident, versions) => {
                return_source_if_disabled!(self, *loc, ';');

                write_chunk!(self, ident.loc.end(), "pragma {}", &ident.name)?;
                ...
                for version in versions.iter() {
                    ...
                    self.visit_version(version.loc(), version)?;
                }
    ...
    #[instrument(name = "version", skip_all)]
    fn visit_version(&mut self, loc: Loc, version: &VersionComparator) -> Result<()> {
        return_source_if_disabled!(self, loc, ';');

        match version {
            VersionComparator::Plain { loc, version } => {
                write_chunk!(self, loc.start(), loc.end(), "{}", version.iter().join("."))?;
            }
            VersionComparator::Operator { loc, op, version } => {
                write_chunk!(
                    self,
                    loc.start(),
                    loc.end(),
            ...
            VersionComparator::Or { loc, left, right } => {
                self.visit_version(*loc, left)?;
                write_chunk!(self, loc.start(), loc.end(), " || ")?;
                self.visit_version(*loc, right)?;
            }
            VersionComparator::Range { loc, from, to } => {
                write_chunk!(
                    self,
                    loc.start(),
                    loc.end(),
    ...
    fn visit_args(&mut self, loc: Loc, args: &mut Vec<NamedArgument>) -> Result<(), Self::Error> {
        ...
        }) = args_iter.next()
        {
            ...
            chunks.push(
                self.chunked(arg_loc.start(), Some(next_byte_offset), |fmt| {
                    fmt.grouped(|fmt| {
        ...
        match expr {
            Expression::Type(loc, typ) => match typ {
                ...
                Type::Int(ref n) | Type::Uint(ref n) => {
                    let int = if matches!(typ, Type::Int(_)) {
                        "int"
                    } else {
                        "uint"
                    };
                ...
                Type::Mapping {
                    ...
                    let close_paren_loc = self
                        .find_next_in_src(value.loc().end(), ')')
                        .unwrap_or(loc.end());
    ...
    fn visit_var_definition_stmt(
        ...
        let declaration = self.chunked(declaration.loc.start(), None, |fmt| {
            fmt.visit_var_declaration(declaration)
        })?;
            ...
            FunctionAttribute::BaseOrModifier(loc, base) => {
                ...
                if is_contract_base {
                    ...
                    if is_lowercase && base_or_modifier.content.ends_with("()") {
                        base_or_modifier
                            .content
                            .truncate(base_or_modifier.content.len() - 2);
                    }
    ...
    fn visit_event(&mut self, event: &mut EventDefinition) -> Result<()> {
        ...
        let last_chunk = if event.anonymous {
            ") anonymous;"
        } else {
            ");"
        };
    ...
    fn visit_using(&mut self, using: &mut Using) -> Result<()> {
        ...
        let (is_library, mut list_chunks) = match &mut using.list {
            UsingList::Library(library) => (
                true,
                vec![self.visit_to_chunk(library.loc.start(), None, library)?],
            ),
        ...
        let ty_chunk = if let Some(ty) = &mut using.ty {
            ...
            self.chunk_at(
                using.loc.start(),
                Some(global_start.unwrap_or(loc_end)),
                None,
                "*",
            )
        };
    ...
    fn visit_yul_for(&mut self, stmt: &mut YulFor) -> Result<(), Self::Error> {
        ...
        self.visit_yul_block(
            stmt.execution_block.loc,
            &mut stmt.execution_block.statements,
            true,
        )?;
        Ok(())
    }
